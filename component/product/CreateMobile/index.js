...
import { BIKE_ADD, GET_MOBILE_BRAND, MOBILE_ADD } from "@/store/Services/api";

const CreateMobile = () => {
  const [mobileBrand, setMobileBrand] = useState("");
  const [fieldMobileBrand, setFieldMobileBrand] = useState("");
  ...
  const [field_mobile_images, setField_mobile_images] = useState("");
  ...
  let get_MobileBrandValue = [];
  if (mobileBrand && mobileBrand.length) {
    mobileBrand.map((get_mobileBrand, e) => {
      get_MobileBrandValue.push({
        key: e,
        value: get_mobileBrand.name,
        text: get_mobileBrand.name,
      });
    });
  }

  // car make
  const MobileBrandHandle = () => {
    Http.get(GET_MOBILE_BRAND).then((res) => {
      const data = res.data.mobile_brand;
      setMobileBrand(data);
    });
  };
  const SelectMobileBrandHandle = (e) => {
    const SelectMobileBrandVal = e.target.innerText;
    setFieldMobileBrand(SelectMobileBrandVal);
  };
  console.log("eee", fieldMobileBrand);
  ...
  const adsSubmithandle = (e) => {
    console.log("fieldMobileBrand", fieldMobileBrand, field_title);
    const fileData = new FormData();
    fileData.append("category_id", "3");
    fileData.append("user_id", userId);
    fileData.append("mobile_images", field_mobile_images);
    fileData.append("title", field_title);
    fileData.append("description", field_description);
    fileData.append("mobile_brand", fieldMobileBrand);
  ...
  const ImageHandle = (e) => {
    ...
    setField_mobile_images(e.target.files[0]);
  };
        ...
        <Container text>
          {status == 200 ? (
            ...
            <Form className={"pb-20"}>
              ...
              <div>
                <label className={styles.labelfirst}>Mobile Brand</label>
                <Form.Field
                  control={Select}
                  Dropdown
                  placeholder="select mobile brand"
                  search
                  selection
                  value={get_MobileBrandValue.value}
                  options={get_MobileBrandValue}
                  onChange={(e) => setFieldMobileBrand(e.target.innerText)}
                />
              </div>
              <div>
                ...
                <div className="flexbox">
                  <label>
                    ...
                    />
                    <div class={styles.radioLabel}>Used Mobile</div>
                  </label>
                  <label>
                    <input
                      type={"radio"}
                      ...
                      onChange={(e) => setField_condition(e.target.value)}
                    />
                    <div class={styles.radioLabel}>New Mobile</div>
                  </label>
                </div>
              </div>
              <hr />
              ...
              {/* <Form.Field
                  ...
                  onChange={(e)=> setField_mobile_images(e.target.value)}
               /> */}
              <div>
